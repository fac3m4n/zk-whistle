...
(Listing~\ref{lst:irys-client}).

\begin{lstlisting}[language=TypeScript,caption={Thin storage client (\code{irysUpload.ts}).},label={lst:irys-client}]
...
The server route is where the security controls live (Listing~\ref{lst:irys-route}).
...
\begin{lstlisting}[language=TypeScript,caption={Hardened upload route (abridged) (\code{api/storage/route.ts}).},label={lst:irys-route}]
